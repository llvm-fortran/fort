...
bool Sema::ActOnObjectArraySpec(ASTContext &C, SourceLocation Loc, DeclSpec &DS,
                                ArrayRef<ArraySpec *> Dimensions) {
  if (!DS.hasAttributeSpec(DeclSpec::AS_dimension))
    DS.setAttributeSpec(DeclSpec::AS_dimension);
  DS.setDimensions(Dimensions);
...
void Sema::ActOnTypeDeclSpec(ASTContext &C, SourceLocation Loc,
  ...
  if (!D) {
    Diags.Report(Loc, diag::err_undeclared_var_use) << IDInfo;
    return;
  }
  auto Record = dyn_cast<RecordDecl>(D);
  if (!Record) {
    Diags.Report(Loc, diag::err_use_of_not_typename) << IDInfo;
    return;
  }
  DS.setRecord(Record);
}

void Sema::DiagnoseRedefinition(SourceLocation Loc,
                                const IdentifierInfo *IDInfo, Decl *Prev) {
  Diags.Report(Loc, diag::err_redefinition) << IDInfo;
  Diags.Report(Prev->getLocation(), diag::note_previous_definition);
}

bool Sema::CheckDeclaration(const IdentifierInfo *IDInfo,
...
QualType Sema::ActOnTypeName(ASTContext &C, DeclSpec &DS) {
  ...
  case DeclSpec::TST_character:
    if (DS.isStarLengthSelector())
      ...
      Result = QualType(C.getCharacterType(EvalAndCheckCharacterLength(
  ...
  if (Kind != Type::NoKind || DS.isDoublePrecision() || DS.isByte()) {
    ...
    case DeclSpec::TST_real:
      Result =
          Kind == Type::NoKind
              ? (DS.isDoublePrecision() ? C.DoublePrecisionTy : C.RealTy)
              : QualType(C.getBuiltinType(BuiltinType::Real, Kind, true), 0);
      break;
    case DeclSpec::TST_logical:
      Result =
          Kind == Type::NoKind
              ? (DS.isByte() ? C.ByteTy : C.LogicalTy)
              : QualType(C.getBuiltinType(BuiltinType::Logical, Kind, true), 0);
      break;
    case DeclSpec::TST_complex:
      Result =
          Kind == Type::NoKind
              ? (DS.isDoublePrecision() ? C.DoubleComplexTy : C.ComplexTy)
...
static QualType getTypeWithAttribute(ASTContext &C, QualType T,
                                     Qualifiers::AS AS) {
  auto Split = T.split();
...
Decl *Sema::ActOnExternalEntityDecl(ASTContext &C, QualType T,
  ...
  auto Decl =
      FunctionDecl::Create(C,
                           ArgumentExternal ? FunctionDecl::ExternalArgument
                                            : FunctionDecl::External,
                           CurContext, DeclName, T);
  SetFunctionType(Decl, T, TypeLoc,
...
Decl *Sema::ActOnIntrinsicEntityDecl(ASTContext &C, QualType T,
  ...
  auto Decl = IntrinsicFunctionDecl::Create(C, CurContext, IDLoc, IDInfo, Type,
                                            FuncResult.Function);
  CurContext->addDecl(Decl);
  return Decl;
}
...
Decl *Sema::ActOnParameterEntityDecl(ASTContext &C, QualType T,
  ...
  if (VD) {
    Value = CheckAndApplyAssignmentConstraints(
        EqualLoc, VD->getType(), Value.get(),
  ...
  if (auto Prev = LookupIdentifier(IDInfo)) {
    ...
    if (FD && (FD->isNormalFunction() || FD->isExternal())) {
      if (FD->getType().isNull() || FD->getType()->isVoidType()) {
        SetFunctionType(FD, T, IDLoc,
                        SourceRange()); // Fixme: proper loc and range
        return FD;
      } else {
        Diags.Report(IDLoc, diag::err_func_return_type_already_specified)
            << IDInfo;
        return nullptr;
      }
    }
...
FieldDecl *Sema::ActOnDerivedTypeFieldDecl(ASTContext &C, DeclSpec &DS,
                                           SourceLocation IDLoc,
                                           const IdentifierInfo *IDInfo,
                                           ExprResult Init) {
  QualType T = ActOnTypeName(C, DS);
...
void Sema::ActOnENDTYPE(ASTContext &C, SourceLocation Loc, SourceLocation IDLoc,
                        const IdentifierInfo *IDInfo) {
  ...
  if (IDInfo && IDInfo != Record->getIdentifier()) {
    Diags.Report(IDLoc, diag::err_expected_type_name)
        << Record->getIdentifier() << /*Kind=*/0 << getTokenRange(IDLoc);
  }
}

void Sema::ActOnEndDerivedTypeDecl(ASTContext &C) { PopDeclContext(); }
