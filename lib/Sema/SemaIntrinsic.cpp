...
bool Sema::CheckIntrinsicConversionFunc(intrinsic::FunctionKind Function,
  ...
  case IDINT:
    if (Function == IFIX)
      CheckStrictlyRealArgument(Item, true);
    else if (Function == IDINT)
      CheckDoublePrecisionRealArgument(Item, true);
    else
      CheckIntegerOrRealOrComplexArgument(Item, true);
    ReturnType =
        GetUnaryReturnType(Item, Kind ? ApplyTypeKind(Context.IntegerTy, Kind)
                                      : Context.IntegerTy);
    break;
  ...
  case SNGL: {
    if (Function == FLOAT)
      CheckIntegerArgument(Item, true);
    ...
    if (!Kind && ItemType->isComplexType()) {
      ReturnType =
          GetUnaryReturnType(Item, Context.getComplexTypeElementType(ItemType));
      break;
    }
    ReturnType = GetUnaryReturnType(
        Item, Kind ? ApplyTypeKind(Context.RealTy, Kind) : Context.RealTy);
    break;
  }
  case DBLE:
    CheckIntegerOrRealOrComplexArgument(Item, true);
  ...
  case DCMPLX:
    ...
    ReturnType = GetUnaryReturnType(
        Item, Kind ? ApplyTypeKind(Context.ComplexTy, Kind)
...
bool Sema::CheckIntrinsicComplexFunc(intrinsic::FunctionKind Function,
  ...
  if (GenericFunction != Function) {
    if (CheckDoubleComplexArgument(Arg, true))
      return true;
  } else {
    if (CheckComplexArgument(Arg, true))
      return true;
  }
  ...
  case AIMAG:
    ReturnType = GetUnaryReturnType(
        Arg, Context.getComplexTypeElementType(
                 Arg->getType().getSelfOrArrayElementType()));
    break;
  case CONJG:
    ReturnType = Arg->getType();
    break;
...
bool Sema::CheckIntrinsicMathsFunc(intrinsic::FunctionKind Function,
  ...
  case ABS:
    if (GenericFunction != Function) {
      ...
      case DABS:
        ...
        break;
      case CABS:
        CheckComplexArgument(FirstArg, true);
        break;
      case CDABS:
    ...
    if (FirstArg->getType()->isComplexType()) {
      ReturnType = GetUnaryReturnType(
          FirstArg, TypeWithKind(Context, Context.RealTy, FirstArg->getType()));
    } else
  ...
  case DPROD:
    ...
    CheckArrayArgumentsDimensionCompability(FirstArg, SecondArg, "x", "y");
    ReturnType =
        GetBinaryReturnType(FirstArg, SecondArg, Context.DoublePrecisionTy);
    break;
  ...
  case MINLOC:
    ...
    if (SecondArg) {
      ...
      if (SecondArg->getType()->isIntegerType()) {
        ...
        CheckArrayArgumentsDimensionCompability(FirstArg, SecondArg, "array",
                                                "mask");
      else {
        if (ArrayDimCount == 1) {
          ReturnType = Context.IntegerTy;
...
bool Sema::CheckIntrinsicSystemFunc(intrinsic::FunctionKind Function,
  ...
  case ETIME:
    if (!CheckStrictlyRealArrayArgument(FirstArg, "tarray"))
      CheckArrayArgumentDimensionCompability(
