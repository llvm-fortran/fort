...
static StmtLabelInteger GetStmtLabelValue(const Expr *E) {
  if (const IntegerConstantExpr *IExpr = dyn_cast<IntegerConstantExpr>(E)) {
    return StmtLabelInteger(IExpr->getValue().getLimitedValue(
...
Stmt *StmtLabelScope::Resolve(Expr *StmtLabel) const {
  ...
  if (Result == StmtLabelDeclsInScope.end())
    return nullptr;
...
void StmtLabelScope::RemoveForwardReference(const Stmt *User) {
  for (size_t I = 0; I < ForwardStmtLabelDeclsInScope.size(); ++I) {
    if (ForwardStmtLabelDeclsInScope[I].Statement == User) {
      ForwardStmtLabelDeclsInScope.erase(ForwardStmtLabelDeclsInScope.begin() +
                                         I);
      return;
    }
  }
...
void ConstructNameScope::setParent(ConstructNameScope *P) { Parent = P; }

void ConstructNameScope::Declare(const IdentifierInfo *Name,
                                 NamedConstructStmt *Construct) {
  Constructs.insert(std::make_pair(Name, Construct));
}

NamedConstructStmt *
ConstructNameScope::Resolve(const IdentifierInfo *ConstructName) const {
  auto Result = Constructs.find(ConstructName);
  if (Result == Constructs.end())
    return nullptr;
  return Result->second;
}
...
bool ImplicitTypingScope::Apply(const ImplicitStmt::LetterSpecTy &Spec,
                                QualType T) {
  if (None)
    return false;
...
ImplicitTypingScope::Resolve(const IdentifierInfo *IdInfo) {
  ...
  if (Result != Rules.end())
    ...
    return std::make_pair(DefaultRule, QualType());
}

InnerScope::InnerScope(InnerScope *Prev) : Parent(Prev) {}
...
CommonBlockScope::CommonBlockScope() : UnnamedBlock(nullptr) {}
