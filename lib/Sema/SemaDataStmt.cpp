...
public:
  DataValueIterator(ArrayRef<Expr *> Vals) : Values(Vals), ValueOffset(0) {
    InitItem();
  }

  QualType getType() const { return Value->getType(); }

  Expr *getValue() const { return Value; }

  Expr *getActualValue() const { return Values[ValueOffset]; }

  Expr *getActualLastValue() const { return Values.back(); }

  bool isEmpty() const { return ValueOffset >= Values.size(); }
...
public:
  DataStmtEngine(DataValueIterator &Vals, fort::Sema &S,
      ...
      : Values(Vals), Sem(S), Context(S.getContext()), Diags(Diag),
        DataStmtLoc(Loc), ImpliedDoEvaluator(), Done(false) {}
  ...
  void CreateArrayElementExprInitializer(ArrayElementExpr *E,
                                         Expr *Parent = nullptr);
  ...
  ExprResult
  CreateMemberExprInitializer(const MemberExpr *E,
...
bool DataStmtEngine::CheckVar(VarExpr *E) {
  ...
  if (VD->isArgument() || VD->isParameter() || VD->isFunctionResult()) {
...
ExprResult DataStmtEngine::getAndCheckValue(QualType LHSType, const Expr *LHS) {
  if (!HasValues(LHS))
    return ExprResult(true);
  auto Value = Values.getValue();
  Values.advance();
  return Sem.CheckAndApplyAssignmentConstraints(
      Value->getLocation(), LHSType, Value,
      Sema::AssignmentAction::Initializing, LHS);
}

ExprResult DataStmtEngine::getAndCheckAnyValue(QualType LHSType,
                                               const Expr *LHS) {
  ...
  if (ET->isCharacterType() && Val.isUsable()) {
    ...
    return cast<CharacterConstantExpr>(Val.get())
        ->CreateCopyWithCompatibleLength(Context, ET);
  }
  return Val;
}
...
void DataStmtEngine::VisitVarExpr(VarExpr *E) {
  ...
  if (auto ATy = Type->asArrayType()) {
    ...
    if (IsUsable) {
      VD->setInit(ArrayConstructorExpr::Create(Context, Loc, Items, Type));
    }
    return;
  }
    ...
    if (auto SE = dyn_cast<SubstringExpr>(Parent)) {
      ...
      if (Offset < Items.size()) {
        const TypeConstructorExpr *Init =
            Items[Offset] ? cast<TypeConstructorExpr>(Items[Offset]) : nullptr;
        Val = CreateMemberExprInitializer(ME, Init);
      }
    } else
      llvm_unreachable("invalid expression");
  } else
    Val = getAndCheckAnyValue(ElementType, E);
...
ExprResult
DataStmtEngine::CreateMemberExprInitializer(const MemberExpr *E,
                                            const TypeConstructorExpr *Init) {
...
void DataStmtEngine::VisitMemberExpr(MemberExpr *E) {
  ...
  auto VD = Target->getVarDecl();
  const TypeConstructorExpr *Init =
      VD->hasInit() ? cast<TypeConstructorExpr>(VD->getInit()) : nullptr;
...
StmtResult Sema::ActOnDATA(ASTContext &C, SourceLocation Loc,
  ...
  auto Result = DataStmt::Create(C, Loc, Objects, Values, StmtLabel);
  if (StmtLabel)
    DeclareStatementLabel(StmtLabel, Result);
  return Result;
}

ExprResult Sema::ActOnDATAConstantExpr(ASTContext &C, SourceLocation RepeatLoc,
  ...
  if (RepeatCount.isUsable()) {
    ...
    if (!RepeatExpr || !RepeatExpr->getValue().isStrictlyPositive()) {
  ...
  if (HasErrors)
    return ExprError();
  return RepeatExpr ? RepeatedConstantExpr::Create(C, RepeatLoc, RepeatExpr,
                                                   Value.take())
                    : Value;
}
