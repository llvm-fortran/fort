...
int64_t Sema::EvalAndCheckIntExpr(const Expr *E, int64_t ErrorValue) {
  auto Type = E->getType();
...
BuiltinType::TypeKind Sema::EvalAndCheckTypeKind(QualType T, const Expr *E) {
  ...
  case BuiltinType::Complex: {
    switch (Result) {
    case 4:
      return BuiltinType::Real4;
    case 8:
      return BuiltinType::Real8;
    case 16:
      return BuiltinType::Real16;
    }
    break;
  }
  default:
...
QualType Sema::ApplyTypeKind(QualType T, const Expr *E) {
  ...
  return Kind != BuiltinType::NoKind
             ? QualType(Context.getBuiltinType(
                            T->asBuiltinType()->getTypeSpec(), Kind, true),
                        0)
             : T;
}
...
bool Sema::CheckConstantExpression(const Expr *E) {
  if (!E->isEvaluatable(Context)) {
    Diags.Report(E->getLocation(), diag::err_expected_constant_expr)
        << E->getSourceRange();
    return false;
  }
  return true;
...
bool Sema::CheckIntegerOrRealConstantExpression(const Expr *E) {
  auto T = E->getType();
  if (!(T->isIntegerType() || T->isRealType()) || !E->isEvaluatable(Context)) {
...
class ArgumentDependentExprChecker
    : public ExprVisitor<ArgumentDependentExprChecker> {
public:
  ASTContext &Context;
  Sema &Sem;
  ...
  ArgumentDependentExprChecker(ASTContext &C, Sema &S, DiagnosticsEngine &Diag)
      : Context(C), Sem(S), Diags(Diag), HasArgumentTypeErrors(false),
    ...
    if (VD->isArgument()) {
      ...
      if (!CheckEvaluatable) {
        ...
        if (!VD->getType()->isIntegerType()) {
          Diags.Report(E->getLocation(),
                       diag::err_array_explicit_shape_requires_int_arg)
  ...
  void VisitUnaryExpr(UnaryExpr *E) { Visit(E->getExpression()); }
  void VisitImplicitCastExpr(ImplicitCastExpr *E) { Visit(E->getExpression()); }
...
bool Sema::StatementRequiresConstantExpression(SourceLocation Loc,
                                               const Expr *E) {
  if (!E->isEvaluatable(Context)) {
    Diags.Report(Loc, diag::err_stmt_requires_consant_expr)
        << E->getSourceRange();
    return false;
  }
  return true;
...
bool Sema::CheckIntegerExpression(const Expr *E) {
  if (E->getType()->isIntegerType())
    return true;
  Diags.Report(E->getLocation(), diag::err_typecheck_expected_integer_expr)
...
bool Sema::CheckScalarNumericExpression(const Expr *E) {
  if (E->getType()->isIntegerType() || E->getType()->isRealType())
    return true;
  Diags.Report(E->getLocation(), diag::err_expected_scalar_numeric_expr)
      << E->getType() << E->getSourceRange();
  return false;
}
...
bool Sema::StmtRequiresIntegerVar(SourceLocation Loc, const VarExpr *E) {
  if (E->getType()->isIntegerType())
    return true;
  Diags.Report(Loc, diag::err_typecheck_stmt_requires_int_var)
      << E->getType() << E->getSourceRange();
  return false;
}

bool Sema::StmtRequiresScalarNumericVar(SourceLocation Loc, const VarExpr *E,
                                        unsigned DiagId) {
  if (E->getType()->isIntegerType() || E->getType()->isRealType())
    return true;
  Diags.Report(Loc, DiagId) << E->getType() << E->getSourceRange();
  return false;
}
...
bool Sema::CheckLogicalExpression(const Expr *E) {
  if (E->getType()->isLogicalType())
    return true;
  Diags.Report(E->getLocation(), diag::err_typecheck_expected_logical_expr)
      << E->getType() << E->getSourceRange();
  return false;
}
...
bool Sema::StmtRequiresLogicalArrayExpression(SourceLocation Loc,
                                              const Expr *E) {
  auto T = E->getType();
  if (T->isArrayType()) {
...
bool Sema::StmtRequiresIntegerOrLogicalOrCharacterExpression(SourceLocation Loc,
                                                             const Expr *E) {
  auto Type = E->getType();
...
bool Sema::CheckCharacterExpression(const Expr *E) {
  if (E->getType()->isCharacterType())
    return true;
  Diags.Report(E->getLocation(), diag::err_typecheck_expected_char_expr)
      << E->getType() << E->getSourceRange();
  return false;
}
...
bool Sema::CheckTypesOfSameKind(QualType A, QualType B, const Expr *E) const {
  if (AreTypesOfSameKind(A, B))
    return true;
...
bool Sema::CheckTypeScalarOrCharacter(const Expr *E, QualType T,
                                      bool IsConstant) {
  if (T->isBuiltinType() || T->isCharacterType())
    return true;
  Diags.Report(E->getLocation(),
               IsConstant ? diag::err_expected_scalar_or_character_constant_expr
...
Expr *
Sema::TypecheckExprIntegerOrLogicalOrSameCharacter(Expr *E,
  ...
  if (ExpectedType->isIntegerType()) {
    if (CheckIntegerExpression(E)) {
      if (!AreTypesOfSameKind(ExpectedType, GivenType))
        return ImplicitCastExpr::Create(Context, E->getLocation(), ExpectedType,
                                        E);
    }
    ...
    if (!GivenType->isCharacterType() ||
        !AreTypesOfSameKind(ExpectedType, GivenType)) {
      Diags.Report(E->getLocation(), diag::err_typecheck_expected_char_expr)
          << E->getType() << E->getSourceRange();
    }
  }
...
bool Sema::IsDefaultBuiltinOrDoublePrecisionType(QualType T) {
  ...
  if (BTy->isDoublePrecisionKindSpecified() || BTy->isByteKindSpecified() ||
...
bool Sema::CheckDefaultBuiltinOrDoublePrecisionExpression(const Expr *E) {
  if (IsDefaultBuiltinOrDoublePrecisionType(E->getType()))
    return true;
  Diags.Report(E->getLocation(), diag::err_typecheck_expected_default_kind_expr)
      << E->getType() << E->getSourceRange();
  return false;
}
...
static const BuiltinType *getBuiltinType(const Expr *E,
  ...
  return dyn_cast<BuiltinType>(
      ScalarizeType(E->getType(), AllowArrays).getTypePtr());
}
...
bool Sema::DiagnoseIncompatiblePassing(const Expr *E, QualType T,
                                       bool AllowArrays, StringRef ArgName) {
  QualType ET =
      AllowArrays ? E->getType().getSelfOrArrayElementType() : E->getType();
  if (ArgName.empty())
    Diags.Report(E->getLocation(), diag::err_typecheck_passing_incompatible)
        << ET << T << E->getSourceRange();
  else
    Diags.Report(E->getLocation(),
                 diag::err_typecheck_passing_incompatible_named_arg)
...
bool Sema::CheckArgumentsTypeCompability(const Expr *E1, const Expr *E2,
  ...
  Diags.Report(E2->getLocation(), diag::err_typecheck_arg_conflict_type)
      << ArgName1 << ArgName2 << T1 << T2 << E1->getSourceRange()
      << E2->getSourceRange();
  return true;
}
...
bool Sema::CheckDoublePrecisionRealArgument(const Expr *E, bool AllowArrays) {
  if (!IsTypeDoublePrecisionReal(ScalarizeType(E->getType(), AllowArrays)))
    return DiagnoseIncompatiblePassing(E, Context.DoublePrecisionTy,
                                       AllowArrays);
  return false;
}
...
bool Sema::CheckIntegerOrRealOrComplexArgument(const Expr *E,
  ...
  if (!Type || !Type->isIntegerOrRealOrComplexType())
    return DiagnoseIncompatiblePassing(E, "'integer' or 'real' or 'complex'",
                                       AllowArrays);
  return false;
}
...
bool Sema::CheckLogicalArrayArgument(const Expr *E, StringRef ArgName) {
  ...
  Diags.Report(E->getLocation(),
               diag::err_typecheck_passing_incompatible_named_arg)
      << E->getType() << ArgName << "'logical array'" << E->getSourceRange();
  return true;
}

bool Sema::CheckIntegerArgumentOrLogicalArrayArgument(const Expr *E,
                                                      StringRef ArgName1,
                                                      StringRef ArgName2) {
  ...
  Diags.Report(E->getLocation(),
               diag::err_typecheck_passing_incompatible_named_args)
      << E->getType() << ArgName1 << "'integer'" << ArgName2
      << "'logical array'" << E->getSourceRange();
  return true;
}

bool Sema::CheckArrayNoImpliedDimension(const ArrayType *T, SourceRange Range) {
...
bool Sema::CheckArrayExpr(const Expr *E) {
  return CheckArrayNoImpliedDimension(E->getType()->asArrayType(),
                                      E->getSourceRange());
}
...
template <typename T>
static bool CheckArrayCompability(Sema &S, T &Handler, const ArrayType *LHS,
  ...
  if (LHS->getDimensionCount() != RHS->getDimensionCount()) {
    Handler.DimensionCountMismatch(Loc, LHS->getDimensionCount(),
                                   RHS->getDimensionCount(), LHSRange,
                                   RHSRange);
    return false;
  }
  ...
  for (size_t I = 0, Count = LHS->getDimensionCount(); I < Count; ++I) {
    ...
    if (LHSDim->Evaluate(LHSSpec, S.getContext())) {
      if (RHSDim->Evaluate(RHSSpec, S.getContext())) {
        ...
        if (LHSSize != RHSSize) {
          Handler.DimensionSizeMismatch(Loc, LHSSize, I + 1, RHSSize, LHSRange,
                                        RHSRange);
          return false;
        }
      }
...
bool Sema::CheckArrayDimensionsCompability(const ArrayType *LHS,
  ...
  struct Handler {
    ...
    void DimensionCountMismatch(SourceLocation Loc, int LHSDims, int RHSDims,
                                SourceRange LHSRange, SourceRange RHSRange) {
      Diags.Report(Loc, diag::err_typecheck_expected_array_of_dim_count)
          << LHSDims << RHSDims << LHSRange << RHSRange;
    }

    void DimensionSizeMismatch(SourceLocation Loc, int64_t LHSSize, int Dim,
                               int64_t RHSSize, SourceRange LHSRange,
                               SourceRange RHSRange) {
      Diags.Report(Loc, diag::err_typecheck_array_dim_shape_mismatch)
          << Dim << int(LHSSize) << int(RHSSize) << LHSRange << RHSRange;
    }
  };

  Handler Reporter(Diags);
  return CheckArrayCompability(*this, Reporter, LHS, RHS, Loc, LHSRange,
                               RHSRange);
}

bool Sema::CheckArrayArgumentDimensionCompability(const Expr *E,
                                                  const ArrayType *AT,
                                                  StringRef ArgName) {
  ...
  struct Handler {
    ...
    Handler(DiagnosticsEngine &D, StringRef Name1) : Diags(D), ArgName(Name1) {}

    void DimensionCountMismatch(SourceLocation Loc, int LHSDims, int RHSDims,
                                SourceRange LHSRange, SourceRange RHSRange) {
      Diags.Report(Loc, diag::err_typecheck_arg_conflict_array_dim_count)
          << LHSDims << RHSDims << ArgName << LHSRange;
    }
    ...
    void DimensionSizeMismatch(SourceLocation Loc, int64_t LHSSize, int Dim,
      ...
      Diags.Report(Loc, diag::err_typecheck_arg_conflict_array_dim_size)
          << Dim << int(LHSSize) << int(RHSSize) << ArgName << LHSRange;
    }
  };
...
bool Sema::CheckArrayArgumentsDimensionCompability(const Expr *E1,
                                                   const Expr *E2,
                                                   StringRef ArgName1,
                                                   StringRef ArgName2) {
  ...
  struct Handler {
    ...
    Handler(DiagnosticsEngine &D, StringRef Name1, StringRef Name2)
        : Diags(D), A1(Name1), A2(Name2) {}

    void DimensionCountMismatch(SourceLocation Loc, int LHSDims, int RHSDims,
                                SourceRange LHSRange, SourceRange RHSRange) {
      Diags.Report(Loc, diag::err_typecheck_args_conflict_array_dim_count)
          << LHSDims << RHSDims << A1 << A2 << LHSRange << RHSRange;
    }
    ...
    void DimensionSizeMismatch(SourceLocation Loc, int64_t LHSSize, int Dim,
      ...
      Diags.Report(Loc, diag::err_typecheck_args_conflict_array_dim_size)
          << Dim << int(LHSSize) << int(RHSSize) << A1 << A2 << LHSRange
          << RHSRange;
    }
  };
...
bool Sema::CheckRecursiveFunction(SourceLocation Loc) {
  ...
  if (!Function->isRecursive()) {
    Diags.Report(Loc, diag::err_call_non_recursive)
        << (Function->isSubroutine() ? 1 : 0) << Function->getIdentifier()
        << getTokenRange(Loc);
    return false;
  }
  return true;
