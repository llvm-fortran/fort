...
bool Sema::IsTypeByte(QualType T) const { return Context.isTypeByte(T); }
...
/// i.e. Int and Int
static QualType SelectLargestKindApplyConversions(ASTContext &C, ExprResult &A,
                                                  ExprResult &B, QualType AType,
                                                  QualType BType) {
  ...
  if (AK == BK)
    return AType;
  else if (C.getTypeKindBitWidth(AK) >= C.getTypeKindBitWidth(BK)) {
...
static QualType
TakeTypeSelectLargestKindApplyConversion(ASTContext &C, int Chosen,
  ...
  if (AK == BK || (Chosen == 0 && AKWidth >= BKWidth) ||
  ...
  auto ReturnType =
      C.getQualTypeOtherKind(ChosenType, Chosen == 0 ? BType : AType);
...
static TypecheckAction TypecheckAssignment(ASTContext &Context,
  ...
  bool IsRHSArithmetic = IsRHSInteger || IsRHSReal || IsRHSComplex;
  ...
  // FIXME: kinds
    ...
    if (!RHSRec)
      ...
      if (!(LHSRec->getDecl()->getIdentifier() ==
                RHSRec->getDecl()->getIdentifier() &&
...
bool Sema::DiagnoseAssignmentResult(AssignConvertType ConvTy,
                                    ...
                                    QualType SrcType, const Expr *SrcExpr,
  ...
  }
    {
      if (auto Field = dyn_cast_or_null<FieldDecl>(Action.getDecl())) {
        Diags.Report(Field->getLocation(),
                     diag::note_typecheck_initializing_member_here)
            << Field->getIdentifier();
      }
    }
    break;
...
ExprResult Sema::CheckAndApplyAssignmentConstraints(SourceLocation Loc,
  ...
  if (LHSType->isArrayType()) {
    ...
    if (RHSType->isArrayType()) {
      ...
      if (!CheckArrayDimensionsCompability(
              LHSType->asArrayType(), RHSType->asArrayType(), Loc,
      ...
      auto Action =
          ::fort::TypecheckAssignment(Context, LHSElementType, RHSElementType);
      if (Action == NoAction)
        return RHS;
      else if (Action == ImplicitCastAction)
        return ImplicitCastExpr::Create(
            Context, RHS->getLocation(),
            Context.getArrayType(LHSElementType,
                                 RHSType->asArrayType()->getDimensions()),
            RHS);
      DiagnoseAssignmentResult(Incompatible, Loc, LHSElementType,
                               RHSElementType, RHS, AAction, LHS);
      return ExprError();
    } else
  ...
  if (Action == NoAction)
    ...
    return ImplicitCastExpr::Create(Context, RHS->getLocation(), LHSType, RHS);
  DiagnoseAssignmentResult(Incompatible, Loc, LHSType, RHSType, RHS, AAction,
                           LHS);
  return ExprError();
}
...
static TypecheckAction TypecheckUnaryExpr(UnaryExpr::Operator Op, QualType T,
                                          unsigned &Diagnostic) {
  switch (Op) {
...
ExprResult Sema::ActOnUnaryExpr(ASTContext &C, SourceLocation Loc,
  ...
  if (Action == ErrorAction) {
    Diags.Report(Loc, Diagnostic)
        << EType << SourceRange(Loc, E.get()->getLocEnd());
    return ExprError();
  }
...
// where typeof(x i) == Integer and typeof(x !i) : Real/Complex, Kind =
// Real/Complex where kindof(x i) == kindof(x !i), Kind = x i where typeof(x1
// and x2) == Integer, Kind = largest kind where typeof(x1 and x2) ==
// Real/Complex, Kind = largest kind
...
static void Fortran90ArithmeticBinaryTypingRules(
    ASTContext &C, BinaryExpr::Operator Op, QualType &ReturnType,
    ExprResult &LHS, ExprResult &RHS, QualType LHSType, QualType RHSType,
    TypeSpecifierType LHSTypeSpec, TypeSpecifierType RHSTypeSpec) {
...
ExprResult Sema::ActOnBinaryExpr(ASTContext &C, SourceLocation Loc,
  ...
  case BinaryExpr::Power: {
    ...
    if (getLangOpts().Fortran77) {
      ...
      // NB: Produce warning
      if ((LHSTypeSpec == TST_complex || RHSTypeSpec == TST_complex)) {
        if ((IsTypeDoublePrecisionReal(LHSType) &&
             !IsTypeDoublePrecisionComplex(RHSType)) ||
            (IsTypeDoublePrecisionReal(RHSType) &&
             !IsTypeDoublePrecisionComplex(LHSType))) {
    ...
    Fortran90ArithmeticBinaryTypingRules(C, Op, ReturnType, LHS, RHS, LHSType,
                                         RHSType, LHSTypeSpec, RHSTypeSpec);
  ...
  case BinaryExpr::LessThanEqual: {
    ...
    if (LHSType->isCharacterType() && RHSType->isCharacterType())
      break;
    ...
    // A complex operand is permitted only when the relational operator is .EQ.
    // or .NE.
    if ((LHSTypeSpec == TST_complex || RHSTypeSpec == TST_complex)) {
      ...
      if (getLangOpts().Fortran77) {
        // Fortran 77: The comparison of a double precision value and a complex
        // value is not permitted.
        if (IsTypeDoublePrecisionReal(LHSType) ||
            IsTypeDoublePrecisionReal(RHSType)) {
          Diags.Report(Loc,
                       diag::warn_f77_typecheck_relational_invalid_operands)
              << LHSType << RHSType
              << SourceRange(LHS.get()->getLocStart(), RHS.get()->getLocEnd());
        }
      }
...
typecheckInvalidOperands:
  Diags.Report(Loc, DiagType)
      << LHSType << RHSType << SourceRange(LHS.get()->getSourceRange())
...
bool Sema::CheckSubscriptExprDimensionCount(SourceLocation Loc,
  ...
  if (AT->getDimensionCount() != Arguments.size()) {
    Diags.Report(Loc, diag::err_array_subscript_dimension_count_mismatch)
        << int(AT->getDimensionCount()) << SourceRange(Loc, RParenLoc);
    return false;
  }
  return true;
}

ExprResult Sema::ActOnSubscriptExpr(ASTContext &C, SourceLocation Loc,
  ...
  // FIXME constraint
  // A subscript expression may contain array element references and function
  // references. NB: typecheck only for the valid expressions
  size_t ValidCount = std::min(
      Target->getType()->asArrayType()->getDimensionCount(), Subscripts.size());
  ...
  auto T =
      C.getArrayType(Target->getType()->asArrayType()->getElementType(), Dims);
  return ArraySectionExpr::Create(C, Loc, Target, Subscripts, T);
}

bool Sema::CheckCallArguments(FunctionDecl *Function,
  ...
  if (Function->isExternal()) {
    ...
    if (Function->getArguments().empty()) {
      ...
      for (size_t I = 0; I < Arguments.size(); ++I) {
        auto Arg =
            VarDecl::CreateArgument(Context, Function, Function->getLocation(),
                                    Function->getIdentifier());
        Arg->setType(Arguments[I] ? Arguments[I]->getType() : Context.RealTy);
        Args[I] = Arg;
      }
  ...
  if (Arguments.size() != FunctionArgs.size()) {
    ...
    if (Arguments.size() < FunctionArgs.size()) {
      Diags.Report(Loc, diag::err_typecheck_call_too_few_args)
          ...
          << unsigned(Arguments.size()) << getTokenRange(IDLoc);
    } else {
      auto LocStart = Arguments[FunctionArgs.size()]
                          ? Arguments[FunctionArgs.size()]->getLocStart()
                          : Loc;
      auto LocEnd = Arguments.back() ? Arguments.back()->getLocEnd() : Loc;
...
static bool IsDirectArrayExpr(const Expr *E) { return isa<VarExpr>(E); }
...
ExprResult
Sema::ActOnIntrinsicFunctionCallExpr(ASTContext &C, SourceLocation Loc,
  ...
  return IntrinsicCallExpr::Create(C, Loc, Function, Arguments, ReturnType);
}
...
ExprResult Sema::ActOnTypeConstructorExpr(ASTContext &C, SourceLocation Loc,
  ...
  for (size_t I = 0; I < Fields.size(); ++I, ++ArgumentId) {
    if (ArgumentId >= Arguments.size()) {
      Diags.Report(LParenLoc, diag::err_typecheck_call_too_few_args)
          << /* type constructor= */ 3 << unsigned(Fields.size())
          << unsigned(Arguments.size()) << getTokenRange(Loc);
      break;
    }
    auto Arg = Arguments[ArgumentId];
    auto E = CheckAndApplyAssignmentConstraints(
        Arg->getLocation(), Fields[I]->getType(), Arg,
        AssignmentAction(AssignmentAction::Initializing, Fields[I]));
    if (E.isUsable())
  ...
  if (ArgumentId < Arguments.size()) {
    auto LocStart = Arguments[Fields.size()]
                        ? Arguments[Fields.size()]->getLocStart()
                        : LParenLoc;
...
ExprResult Sema::ActOnStructureComponentExpr(ASTContext &C, SourceLocation Loc,
  ...
  if (Result.first >= Result.second)
    ...
    Field = dyn_cast<FieldDecl>(*Result.first);
