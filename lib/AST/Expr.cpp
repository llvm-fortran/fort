...
SourceLocation ConstantExpr::getLocEnd() const { return MaxLoc; }

IntegerConstantExpr::IntegerConstantExpr(ASTContext &C, SourceRange Range,
                                         llvm::StringRef Data)
    : ConstantExpr(IntegerConstantExprClass, C.IntegerTy, Range.Start,
                   Range.End) {
...
IntegerConstantExpr *
IntegerConstantExpr::Create(ASTContext &C, SourceRange Range, StringRef Data) {
  return new (C) IntegerConstantExpr(C, Range, Data);
}

IntegerConstantExpr *
IntegerConstantExpr::Create(ASTContext &C, SourceRange Range, APInt Value) {
  return new (C) IntegerConstantExpr(C, Range, Value);
}

RealConstantExpr::RealConstantExpr(ASTContext &C, SourceRange Range,
...
ComplexConstantExpr *ComplexConstantExpr::Create(ASTContext &C,
                                                 SourceRange Range, Expr *Real,
                                                 Expr *Imaginary,
                                                 QualType Type) {
...
CharacterConstantExpr::CharacterConstantExpr(char *Str, SourceRange Range,
                                             QualType T)
    : ConstantExpr(CharacterConstantExprClass, T, Range.Start, Range.End),
      Data(Str) {}

CharacterConstantExpr *CharacterConstantExpr::Create(ASTContext &C,
                                                     SourceRange Range,
                                                     StringRef Data,
                                                     QualType T) {
...
LogicalConstantExpr *LogicalConstantExpr::Create(ASTContext &C,
                                                 SourceRange Range,
                                                 llvm::StringRef Data,
                                                 QualType T) {
...
RepeatedConstantExpr *
RepeatedConstantExpr::Create(ASTContext &C, SourceLocation Loc,
...
MemberExpr *MemberExpr::Create(ASTContext &C, SourceLocation Loc, Expr *Target,
...
SourceLocation SubstringExpr::getLocEnd() const {
  if (EndPoint)
    return EndPoint->getLocEnd();
  else if (StartingPoint)
    return StartingPoint->getLocEnd();
  else
    return getLocation();
}
...
ArrayElementExpr::ArrayElementExpr(ASTContext &C, SourceLocation Loc, Expr *E,
    ...
    : DesignatorExpr(ArrayElementExprClass,
                     E->getType()->asArrayType()->getElementType(), Loc, E),
...
ArraySectionExpr::ArraySectionExpr(ASTContext &C, SourceLocation Loc, Expr *E,
...
FunctionRefExpr *FunctionRefExpr::Create(ASTContext &C, SourceRange Range,
  ...
  return new (C) FunctionRefExpr(Range.Start, Range.End, Function,
                                 C.getFunctionType(Function));
}
...
SourceLocation VarExpr::getLocEnd() const { return NameLocEnd; }
...
UnaryExpr *UnaryExpr::Create(ASTContext &C, SourceLocation Loc, Operator Op,
                             Expr *E) {
  return new (C) UnaryExpr(Expr::UnaryExprClass, E->getType(), Loc, Op, E);
}

SourceLocation UnaryExpr::getLocEnd() const { return E->getLocEnd(); }

DefinedUnaryOperatorExpr::DefinedUnaryOperatorExpr(SourceLocation Loc, Expr *E,
                                                   IdentifierInfo *IDInfo)
    : UnaryExpr(Expr::DefinedUnaryOperatorExprClass, E->getType(), Loc, Defined,
                E),
      II(IDInfo) {}

DefinedUnaryOperatorExpr *
DefinedUnaryOperatorExpr::Create(ASTContext &C, SourceLocation Loc, Expr *E,
                                 IdentifierInfo *IDInfo) {
...
SourceLocation BinaryExpr::getLocStart() const { return LHS->getLocStart(); }

SourceLocation BinaryExpr::getLocEnd() const { return RHS->getLocEnd(); }
...
SourceLocation ImplicitCastExpr::getLocEnd() const { return E->getLocEnd(); }

CallExpr::CallExpr(ASTContext &C, SourceLocation Loc, FunctionDecl *Func,
                   ArrayRef<Expr *> Args)
...
IntrinsicCallExpr::IntrinsicCallExpr(ASTContext &C, SourceLocation Loc,
                                     intrinsic::FunctionKind Func,
                                     ArrayRef<Expr *> Args, QualType ReturnType)
    : Expr(IntrinsicCallExprClass, ReturnType, Loc), MultiArgumentExpr(C, Args),
      Function(Func) {}

IntrinsicCallExpr *IntrinsicCallExpr::Create(ASTContext &C, SourceLocation Loc,
                                             intrinsic::FunctionKind Func,
                                             ArrayRef<Expr *> Arguments,
                                             QualType ReturnType) {
  return new (C) IntrinsicCallExpr(C, Loc, Func, Arguments, ReturnType);
}
...
ImpliedDoExpr::ImpliedDoExpr(ASTContext &C, SourceLocation Loc, VarDecl *Var,
                             ArrayRef<Expr *> Body, Expr *InitialParam,
                             Expr *TerminalParam, Expr *IncrementationParam)
    : Expr(ImpliedDoExprClass, QualType(), Loc), DoVar(Var), DoList(C, Body),
      Init(InitialParam), Terminate(TerminalParam),
...
ImpliedDoExpr *ImpliedDoExpr::Create(ASTContext &C, SourceLocation Loc,
  ...
  return new (C) ImpliedDoExpr(C, Loc, DoVar, Body, InitialParam, TerminalParam,
                               IncrementationParam);
}
...
ArrayConstructorExpr::ArrayConstructorExpr(ASTContext &C, SourceLocation Loc,
    ...
    : Expr(ArrayConstructorExprClass, Ty, Loc), MultiArgumentExpr(C, Items) {}

ArrayConstructorExpr *ArrayConstructorExpr::Create(ASTContext &C,
                                                   SourceLocation Loc,
                                                   ArrayRef<Expr *> Items,
                                                   QualType Ty) {
...
TypeConstructorExpr::TypeConstructorExpr(ASTContext &C, SourceLocation Loc,
    ...
    : Expr(TypeConstructorExprClass, T, Loc), MultiArgumentExpr(C, Arguments),
      Record(record) {}

TypeConstructorExpr *TypeConstructorExpr::Create(ASTContext &C,
                                                 SourceLocation Loc,
                                                 const RecordDecl *Record,
                                                 ArrayRef<Expr *> Arguments) {
  return new (C)
      TypeConstructorExpr(C, Loc, Record, Arguments, C.getRecordType(Record));
}
...
RangeExpr::RangeExpr(ExprClass Class, SourceLocation Loc, Expr *First,
                     Expr *Second)
    : Expr(Class, QualType(), Loc), E1(First), E2(Second) {}

RangeExpr *RangeExpr::Create(ASTContext &C, SourceLocation Loc, Expr *First,
                             Expr *Second) {
  return new (C) RangeExpr(RangeExprClass, Loc, First, Second);
}

void RangeExpr::setFirstExpr(Expr *E) { E1 = E; }

void RangeExpr::setSecondExpr(Expr *E) { E2 = E; }

SourceLocation RangeExpr::getLocStart() const {
  if (hasFirstExpr())
    return E1->getLocStart();
  return getLocation();
}
...
StridedRangeExpr::StridedRangeExpr(SourceLocation Loc, Expr *First,
    ...
    : RangeExpr(StridedRangeExprClass, Loc, First, Second), Stride(stride) {}
...
ArraySpec::ArraySpec(ArraySpecKind K) : Kind(K) {}
...
ExplicitShapeSpec *ExplicitShapeSpec::Create(ASTContext &C, Expr *LB,
                                             Expr *UB) {
...
DeferredShapeSpec::DeferredShapeSpec() : ArraySpec(k_DeferredShape) {}
