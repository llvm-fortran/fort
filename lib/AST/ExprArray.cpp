...
public:
  bool VisitArraySectionExpr(const ArraySectionExpr *E) {
    ...
    for (size_t I = 0; I < Subs.size(); ++I) {
      ...
      if (auto Range = dyn_cast<RangeExpr>(Sub)) {
        ...
        if (LastSliced)
          return false;
        if (!Range->hasFirstExpr() && !Range->hasSecondExpr()) {
          // Ok.
        } else
          LastSliced = true;
      } else {
  ...
  bool VisitUnaryExpr(const UnaryExpr *E) { return Visit(E->getExpression()); }

  bool VisitImplicitCastExpr(const ImplicitCastExpr *E) {
    return Visit(E->getExpression());
  }

  bool VisitVarExpr(const VarExpr *E) { return true; }
};
