...
llvm::Value *ScalarExprEmitter::EmitExpr(const Expr *E) { return Visit(E); }
...
llvm::Value *
CodeGenFunction::ConvertLogicalValueToLogicalMemoryValue(llvm::Value *Val,
                                                         QualType T) {
...
llvm::Value *
CodeGenFunction::EmitIntegerConstantExpr(const IntegerConstantExpr *E) {
  return llvm::ConstantInt::get(Builder.getInt32Ty(),
                                E->getValue().sextOrTrunc(32));
}

llvm::Value *
ScalarExprEmitter::VisitIntegerConstantExpr(const IntegerConstantExpr *E) {
  return CGF.EmitIntegerConstantExpr(E);
}

llvm::Value *
ScalarExprEmitter::VisitRealConstantExpr(const RealConstantExpr *E) {
  return llvm::ConstantFP::get(VMContext, E->getValue());
}

llvm::Value *
ScalarExprEmitter::VisitLogicalConstantExpr(const LogicalConstantExpr *E) {
...
llvm::Value *CodeGenFunction::EmitScalarUnaryMinus(llvm::Value *Val) {
  return Val->getType()->isIntegerTy() ? Builder.CreateNeg(Val)
...
llvm::Value *ScalarExprEmitter::VisitBinaryExpr(const BinaryExpr *E) {
  ...
  if (Op < BinaryExpr::Plus) {
    ...
    // Character comparison
      ...
      return CGF.EmitCharacterRelationalExpr(
          Op, CGF.EmitCharacterExpr(E->getLHS()),
...
llvm::Value *CodeGenFunction::EmitScalarBinaryExpr(BinaryExpr::Operator Op,
  ...
  case BinaryExpr::Plus:
    Result = IsInt ? Builder.CreateAdd(LHS, RHS) : Builder.CreateFAdd(LHS, RHS);
    break;
  case BinaryExpr::Minus:
    Result = IsInt ? Builder.CreateSub(LHS, RHS) : Builder.CreateFSub(LHS, RHS);
    break;
  case BinaryExpr::Multiply:
    Result = IsInt ? Builder.CreateMul(LHS, RHS) : Builder.CreateFMul(LHS, RHS);
    break;
  case BinaryExpr::Divide:
    Result =
        IsInt ? Builder.CreateSDiv(LHS, RHS) : Builder.CreateFDiv(LHS, RHS);
    break;
  case BinaryExpr::Power: {
    ...
    auto Func = GetIntrinsicFunction(Intrinsic, LHS->getType());
...
ConvertRelationalOpToPredicate(BinaryExpr::Operator Op, bool IsInt = false) {
  switch (Op) {
  case BinaryExpr::Eqv:
...
llvm::Value *CodeGenFunction::EmitScalarRelationalExpr(BinaryExpr::Operator Op,
                                                       llvm::Value *LHS,
                                                       llvm::Value *RHS) {
...
llvm::Value *CodeGenFunction::EmitComplexRelationalExpr(BinaryExpr::Operator Op,
                                                        ComplexValueTy LHS,
                                                        ComplexValueTy RHS) {
...
CodeGenFunction::ConvertComparisonResultToRelationalOp(BinaryExpr::Operator Op,
                                                       llvm::Value *Result) {
  return Builder.CreateICmp(ConvertRelationalOpToPredicate(Op, true), Result,
                            Builder.getInt32(0));
}
...
llvm::Value *
ScalarExprEmitter::VisitImplicitCastExpr(const ImplicitCastExpr *E) {
  auto Input = E->getExpression();
  if (Input->getType()->isComplexType())
    return CGF.EmitComplexToScalarConversion(
        CGF.EmitComplexExpr(Input), E->getType().getSelfOrArrayElementType());
  return CGF.EmitScalarToScalarConversion(
...
llvm::Value *
ScalarExprEmitter::VisitIntrinsicCallExpr(const IntrinsicCallExpr *E) {
  return CGF.EmitIntrinsicCall(E).asScalar();
}

llvm::Value *
ScalarExprEmitter::VisitArrayElementExpr(const ArrayElementExpr *E) {
...
llvm::Value *CodeGenFunction::EmitBitOperation(intrinsic::FunctionKind Op,
  ...
  case intrinsic::BTEST: {
    ...
    return Builder.CreateSelect(
        Builder.CreateICmpNE(Result, llvm::ConstantInt::get(A1->getType(), 0)),
  ...
  case intrinsic::IBITS: {
    ...
    auto Mask = Builder.CreateSub(
        Builder.CreateShl(MaskOne,
                          Builder.CreateZExtOrTrunc(A3, A1->getType())),
        MaskOne);
    return Builder.CreateAnd(
        Builder.CreateLShr(A1, Builder.CreateZExtOrTrunc(A2, A1->getType())),
        Mask);
  }
    ...
    if (auto Val = dyn_cast<llvm::ConstantInt>(A2)) {
      if (Val->isNegative())
        return Builder.CreateLShr(
            A1, Builder.CreateZExtOrTrunc(Builder.CreateNeg(A2), A1Ty));
      return Builder.CreateShl(A1, Builder.CreateZExtOrTrunc(A2, A1Ty));
    }
    return Builder.CreateSelect(
        Builder.CreateICmpSGE(A2, llvm::ConstantInt::get(A2->getType(), 0)),
