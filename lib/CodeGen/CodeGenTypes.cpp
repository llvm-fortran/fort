...
llvm::Type *CodeGenTypes::ConvertType(QualType T) {
  ...
  if (const BuiltinType *BTy = dyn_cast<BuiltinType>(TPtr))
    return ConvertBuiltInType(BTy->getTypeSpec(), BTy->getBuiltinTypeKind());
...
llvm::Type *CodeGenTypes::ConvertFunctionType(const FunctionType *T) {
  return llvm::PointerType::get(
      GetFunctionType(T->getPrototype())->getFunctionType(), 0);
}
...
const CGFunctionInfo *CodeGenTypes::GetFunctionType(const FunctionDecl *FD) {
  ...
  for (auto I : AdditionalArgTypes)
    ArgTypes.push_back(I);

  // FIXME: fold same infos into one?
  auto Result = CGFunctionInfo::Create(
      Context, llvm::CallingConv::C,
      llvm::FunctionType::get(ReturnType, ArgTypes, false), ArgInfo,
      ReturnInfo);
  return Result;
}

const CGFunctionInfo *CodeGenTypes::GetFunctionType(FortranABI &ABI,
  ...
  for (size_t I = 0; I < Args.size(); ++I) {
    ...
    if (Args[I].isQualType()) {
      ...
      ConvertArgumentType(ArgTypes, AdditionalArgTypes, Args[I].asQualType(),
                          Info);
    } else {
  ...
  auto Result = CGFunctionInfo::Create(
      Context, llvm::CallingConv::C,
      llvm::FunctionType::get(RetType, ArgTypes, false), ArgInfo, ReturnInfo);
  return Result;
}
