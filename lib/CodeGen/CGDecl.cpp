...
public:
  ...
  void VisitVarDecl(const VarDecl *D) { CGF.EmitVarDecl(D); }
};
...
void CodeGenFunction::EmitVarDecl(const VarDecl *D) {
  if (D->isParameter() || D->isArgument() || D->isFunctionResult())
    return;
...
void CodeGenFunction::EmitVarInitializer(const VarDecl *D) {
  ...
  if (T->isArrayType()) {
    ...
    for (size_t I = 0; I < Init.size(); ++I) {
      ...
      EmitStoreCharSameLength(Val, Builder.CreateConstInBoundsGEP1_64(Dest, I),
                              T.getSelfOrArrayElementType());
    }
    return;
  }
...
std::pair<int64_t, int64_t> CodeGenFunction::GetObjectBounds(const VarDecl *Var,
                                                             const Expr *E) {
  auto Size =
      CGM.getDataLayout().getTypeStoreSize(ConvertTypeForMem(Var->getType()));
  uint64_t Offset = 0;
  if (auto Arr = dyn_cast<ArrayElementExpr>(E)) {
    Arr->EvaluateOffset(getContext(), Offset);
    Offset = Offset * CGM.getDataLayout().getTypeStoreSize(
...
CodeGenFunction::EquivSet
CodeGenFunction::EmitEquivalenceSet(const EquivalenceSet *S) {
...
llvm::Value *CodeGenFunction::EmitEquivalenceSetObject(EquivSet Set,
                                                       const VarDecl *Var) {
  // Compute the pointer to the object.
  auto ObjLowBound = LocalVariablesInEquivSets.find(Var)->second;
  auto Ptr = Builder.CreateConstInBoundsGEP1_64(
      Set.Ptr, uint64_t(ObjLowBound - Set.LowestBound));
  auto T = Var->getType();
  return Builder.CreatePointerCast(
      Ptr, llvm::PointerType::get(ConvertTypeForMem(T), 0));
}
...
void CodeGenFunction::EmitCommonBlock(const CommonBlockSet *S) {
  ...
  for (auto Obj : S->getObjects()) {
    if (Obj.Var) {
      LocalVariables.insert(
          std::make_pair(Obj.Var, Builder.CreateStructGEP(nullptr, Ptr, Idx)));
    }
    ++Idx;
  }
}
