...
llvm::Type *
CodeGenTypes::ConvertReturnType(QualType T,
  ...
  ReturnInfo.Type = T;
  if (T->isComplexType()) {
    CGM.getTargetCodeGenInfo().getABIInfo().computeReturnTypeInfo(
        T, ReturnInfo.ABIInfo);
...
void CodeGenTypes::ConvertArgumentType(
...
void CodeGenTypes::ConvertArgumentTypeForReturnValue(
  ...
  if (ReturnInfo.ABIInfo.getKind() == ABIRetInfo::CharacterValueAsArg ||
    ...
    ConvertArgumentType(ArgTypes, AdditionalArgTypes, T,
                        ReturnInfo.ReturnArgInfo);
  }
}

CGFunctionInfo *CGFunctionInfo::Create(ASTContext &C, llvm::CallingConv::ID CC,
...
RValueTy CodeGenFunction::EmitCall(const FunctionDecl *Function,
  ...
  return EmitCall(Callee, FuncInfo, ArgList, Arguments, ReturnsNothing);
}
...
RValueTy CodeGenFunction::EmitCall(llvm::Value *Callee,
  ...
  auto FType = Callee->getType()->isFunctionTy()
                   ? cast<llvm::FunctionType>(Callee->getType())
                   : dyn_cast<llvm::FunctionType>(
                         cast<llvm::PointerType>(Callee->getType())
                             ->getPointerElementType());
  for (size_t I = 0; I < Arguments.size(); ++I)
    EmitCallArg(FType->getParamType(ArgList.getOffset()), ArgList, Arguments[I],
                ArgumentInfo[I]);
  ...
  if (RetABIKind == ABIRetInfo::CharacterValueAsArg)
    ...
    auto ResultTemp =
        CreateTempAlloca(ConvertTypeForMem(RetType), "complex-return");
  ...
  if (ReturnsNothing || RetABIKind == ABIRetInfo::Nothing)
    ...
    if (RetType->isComplexType()) {
      ...
      auto ResultTemp =
          CreateTempAlloca(ConvertTypeForMem(RetType), "agg-return");
...
RValueTy CodeGenFunction::EmitCall(CGFunction Func,
  ...
  auto Result = Builder.CreateCall(Func.getFunction(), ArgList.createValues());
  Result->setCallingConv(FuncInfo->getCallingConv());
...
void CodeGenFunction::EmitCallArg(llvm::Type *T, CallArgList &Args,
  ...
  // NB: cast pointer types when different argument types are used in source
...
void CodeGenFunction::EmitArrayCallArg(CallArgList &Args, const Expr *E,
                                       CGFunctionInfo::ArgInfo ArgInfo) {
  switch (ArgInfo.ABIInfo.getKind()) {
  case ABIArgInfo::Reference:
...
void CodeGenFunction::EmitCallArg(CallArgList &Args, llvm::Value *Value,
                                  CGFunctionInfo::ArgInfo ArgInfo) {
  assert(ArgInfo.ABIInfo.getKind() == ABIArgInfo::Value);
  Args.add(Value);
}

void CodeGenFunction::EmitCallArg(CallArgList &Args, ComplexValueTy Value,
                                  CGFunctionInfo::ArgInfo ArgInfo) {
...
void CodeGenFunction::EmitCallArg(CallArgList &Args, CharacterValueTy Value,
                                  CGFunctionInfo::ArgInfo ArgInfo) {
  assert(ArgInfo.ABIInfo.getKind() != ABIArgInfo::Reference);
  switch (ArgInfo.ABIInfo.getKind()) {
  case ABIArgInfo::Value:
...
CharacterValueTy CodeGenFunction::GetCharacterArg(const VarDecl *Arg) {
  ...
  if (Result == CharacterArgs.end()) {
    ...
    case ABIArgInfo::Expand: {
      ...
      Val = CharacterValueTy(ExpansionInfo.A1, ExpansionInfo.A2);
      break;
    }

    case ABIArgInfo::ExpandCharacterPutLengthToAdditionalArgsAsInt: {
      auto ExpansionInfo = GetExpandedArg(Arg);
      Val = CharacterValueTy(
          ExpansionInfo.A1,
...
public:
  ...
  ~StatementFunctionInliningScope() { CGF->CurInlinedStmtFunc = Previous; }
...
RValueTy
CodeGenFunction::EmitStatementFunctionCall(const FunctionDecl *Function,
                                           ArrayRef<Expr *> Arguments) {
