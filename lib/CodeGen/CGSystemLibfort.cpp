...
public:
  CGLibfortSystemRuntime(CodeGenModule &CGM) : CGSystemRuntime(CGM) {}
...
llvm::Value *CGLibfortSystemRuntime::EmitMalloc(CodeGenFunction &CGF,
                                                llvm::Value *Size) {
...
void CGLibfortSystemRuntime::EmitFree(CodeGenFunction &CGF, llvm::Value *Ptr) {
  ...
  CGF.EmitCallArg(ArgList,
                  Ptr->getType() == CGM.VoidPtrTy
                      ? Ptr
                      : CGF.getBuilder().CreateBitCast(Ptr, CGM.VoidPtrTy),
                  Func.getInfo()->getArguments()[0]);
  CGF.EmitCall(Func.getFunction(), Func.getInfo(), ArgList);
}

llvm::Value *CGLibfortSystemRuntime::EmitETIME(CodeGenFunction &CGF,
