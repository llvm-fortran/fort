...
static const tok::TokenKind AmbiguousEndKeywords[] = {
    // END<...>name
    tok::kw_END, tok::kw_ENDPROGRAM, tok::kw_ENDSUBROUTINE,
    tok::kw_ENDFUNCTION};
...
static const tok::TokenKind AmbiguousTypeKeywords[] = {
    // INTEGERvar
    tok::kw_INTEGER,         tok::kw_REAL,          tok::kw_COMPLEX,
    tok::kw_DOUBLEPRECISION, tok::kw_DOUBLECOMPLEX, tok::kw_LOGICAL,
    tok::kw_CHARACTER,       tok::kw_BYTE};
...
static const tok::TokenKind AmbiguousTopLvlDeclKeywords[] = {
    // PROGRAMname
    tok::kw_PROGRAM, tok::kw_SUBROUTINE, tok::kw_FUNCTION,
...
CommonAmbiguities::CommonAmbiguities() {
  ...
  {
    const KeywordFilter Filters[] = {AmbiguousExecutableStatements(),
                                     KeywordFilter(tok::kw_THEN)};
    MatcherForKeywordsAfterIF = llvm::makeArrayRef(Filters);
  }
  {
    const KeywordFilter Filters[] = {
        AmbiguousTopLevelDeclarationStatements(), AmbiguousTypeStatements(),
        AmbiguousSpecificationStatements(), AmbiguousExecutableStatements(),
        AmbiguousEndStatements()};
    MatcherForTopLevel = llvm::makeArrayRef(Filters);
  }
  {
    const KeywordFilter Filters[] = {
        AmbiguousTypeStatements(), AmbiguousSpecificationStatements(),
        AmbiguousExecutableStatements(), AmbiguousEndStatements()};
...
void Parser::ReLexAmbiguousIdentifier(
    const fixedForm::KeywordMatcher &Matcher) {
...
bool Parser::ExpectAndConsumeFixedFormAmbiguous(tok::TokenKind ExpectedTok,
                                                unsigned Diag,
                                                const char *DiagMsg) {
  if (Features.FixedForm) {
    ...
    ReLexAmbiguousIdentifier(
        fixedForm::KeywordMatcher(fixedForm::KeywordFilter(ExpectedTok)));
  }
  return ExpectAndConsume(ExpectedTok, Diag, DiagMsg);
}
...
void Parser::LookForTopLevelStmtKeyword() {
  ...
  if (Features.FixedForm) {
    if (!AtStartOfStatement && Tok.isAtStartOfStatement())
      return;
    ReLexAmbiguousIdentifier(
        FixedFormAmbiguities.getMatcherForTopLevelKeywords());
  }
}
...
void Parser::LookForSpecificationStmtKeyword() {
  ...
  if (Features.FixedForm) {
    if (!AtStartOfStatement && Tok.isAtStartOfStatement())
      return;
    ReLexAmbiguousIdentifier(
        FixedFormAmbiguities.getMatcherForSpecificationStmtKeywords());
  }
}
...
void Parser::LookForExecutableStmtKeyword(bool AtStartOfStatement) {
  if (Features.FixedForm) {
    if (!AtStartOfStatement && Tok.isAtStartOfStatement())
      return;
    ReLexAmbiguousIdentifier(
        FixedFormAmbiguities.getMatcherForExecutableStmtKeywords());
  }
}
