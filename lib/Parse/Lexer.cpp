...
Lexer::Lexer(llvm::SourceMgr &SM, const LangOptions &features,
             DiagnosticsEngine &D)
...
Lexer::Lexer(llvm::SourceMgr &SM, const LangOptions &features,
             DiagnosticsEngine &D, SourceLocation StartingPoint)
    : Text(D, features), Diags(D), SrcMgr(SM), Features(features), TokStart(0),
      LastTokenWasSemicolon(false) {
...
void Lexer::LineOfText::SetBuffer(const llvm::MemoryBuffer *Buf,
                                  const char *Ptr, bool AtLineStart) {
...
/// with only comments.
bool Lexer::LineOfText::SkipBlankLinesAndComments(unsigned &I,
                                                  const char *&LineBegin,
                                                  bool IgnoreContinuationChar) {
  // Skip blank lines and lines with only comments.
  while (isVerticalWhitespace(*BufPtr) && *BufPtr != '\0')
    ++BufPtr;

  while (I != LanguageOptions.LineLength && isHorizontalWhitespace(*BufPtr) &&
         *BufPtr != '\0')
    ++I, ++BufPtr;
...
void Lexer::LineOfText::SkipFixedFormBlankLinesAndComments(
    unsigned &I, const char *&LineBegin) {
  // Skip blank lines and lines with only comments.
  while (isVerticalWhitespace(*BufPtr) && *BufPtr != '\0')
    ++BufPtr;
...
void Lexer::LineOfText::GetCharacterLiteral(unsigned &I, const char *&LineBegin,
                                            bool &PadAtoms) {
  // Skip blank lines and lines with only comments.
  SkipBlankLinesAndComments(I, LineBegin);
  ...
  // A flag which ignores the next quote after a double quote continuation is
  // found:
  // 'String'&\n&'' <-- the first ' on the second line needs this flag
  bool skipNextQuoteChar = false;
  ++I, ++BufPtr;
  while (true) {
    while (I != LanguageOptions.LineLength && !isVerticalWhitespace(*BufPtr) &&
           *BufPtr != '\0') {
      if (*BufPtr == '"' || *BufPtr == '\'') {
        if (!skipNextQuoteChar) {
          ...
          if (*BufPtr == '&') {
            unsigned currentI = I;
            const char *CurrentBufPtr = BufPtr, *CurrentLineBegin = LineBegin;
      ...
      if (I == LanguageOptions.LineLength)
        break;
      while (I != LanguageOptions.LineLength &&
             isHorizontalWhitespace(*BufPtr) && *BufPtr != '\0')
        ++I, ++BufPtr;
    ...
    if (AmpersandPos)
      ...
      Diags.Report(SourceLocation::getFromPointer(QuoteStart),
...
void Lexer::LineOfText::dump() const { dump(llvm::errs()); }

void Lexer::LineOfText::dump(raw_ostream &OS) const {
  for (SmallVectorImpl<StringRef>::const_iterator I = Atoms.begin(),
                                                  E = Atoms.end();
       I != E; ++I)
    OS << *I;
  OS << '\n';
...
static inline bool isFormatDescriptorBody(unsigned char c) {
  return (CharInfo[c] & (CHAR_LETTER | CHAR_NUMBER | CHAR_PERIOD)) ? true
                                                                   : false;
}
...
static inline bool isHorizontalTab(unsigned char c) { return c == '\t'; }
...
void Lexer::getSpelling(
    const Token &Tok, llvm::SmallVectorImpl<llvm::StringRef> &Spelling) const {
  assert((int)Tok.getLength() >= 0 && "Token character range is bogus!");

  const char *TokStart =
      Tok.isLiteral() ? Tok.getLiteralData() : Tok.getLocation().getPointer();
...
void Lexer::getFixedFormIdentifierSpelling(
  ...
  while (true) {
    while (Len != TokLen) {
      if (!isHorizontalWhitespace(*CurPtr) && !isVerticalWhitespace(*CurPtr)) {
        ++CurPtr, ++Len;
        continue;
      }
      break;
...
void Lexer::getFixedFormLiteralSpelling(
    const Token &Tok, llvm::SmallVectorImpl<llvm::StringRef> &Spelling,
    const char *TokStart, unsigned TokLen) const {
  const char *CurPtr = TokStart;
  const char *Start = TokStart;
...
void Lexer::LexFixedFormIdentifier(const fixedForm::KeywordMatcher &Matcher,
  ...
  while (!Text.empty() && !Text.AtEndOfLine()) {
    ...
    if (Matcher.Matches(StringRef(Token.data(), Token.size()))) {
...
bool Lexer::ScanPossibleF77Operator() {
  ...
  const char *CSBuiltIn[] = {"TRUE", "FALSE", "EQ",  "NE", "LT",  "LE",   "GT",
                             "GE",   "NOT",   "AND", "OR", "EQV", "NEQV", NULL};
...
bool Lexer::LexPossibleDefinedOperator(Token &Result) {
  ...
  if (Char != '.') {
    Diags.Report(SourceLocation::getFromPointer(TokStart),
                 diag::err_defined_operator_missing_end);
    FormTokenWithChars(Result, tok::unknown);
    return true;
  }
...
void Lexer::LexNumericConstant(Token &Result) {
  ...
  if (BeginsWithDot) {
    ...
    // Could be part of a defined operator. Form numeric constant from what we
    // now have.
    C = getCurrentChar();
...
make_literal:
  if (!IsReal)
    ...
    FormTokenWithChars(Result, IsDoublePrecision
                                   ? tok::double_precision_literal_constant
...
/// constant (string).
void Lexer::LexCharacterLiteralConstant(Token &Result, bool DoubleQuotes) {
  while (true) {
    char C = getNextChar();
    if (C == '\0')
      break;
...
void Lexer::LexComment(Token &Result) {
  ...
  Result.setLiteralData(TokStart);
  for (std::vector<CommentHandler *>::const_iterator I =
...
void Lexer::LexFixedFormIdentifierMatchLongestKeyword(
...
bool FormatDescriptorLexer::LexIntIfPresent(llvm::StringRef &Result) {
  if (IsIntPresent()) {
    auto Start = Offset;
    while (!IsDone() && isDecimalNumberBody(Text[Offset]))
      ++Offset;
...
bool FormatDescriptorLexer::LexIdentIfPresent(llvm::StringRef &Result) {
  if (!IsDone() && isLetter(Text[Offset])) {
    auto Start = Offset;
    while (!IsDone() && isLetter(Text[Offset]))
      ++Offset;
    Result = llvm::StringRef(Text).slice(Start, Offset);
    return true;
  }
...
/// given string.
bool FormatDescriptorLexer::IsDone() const { return Offset >= Text.size(); }
