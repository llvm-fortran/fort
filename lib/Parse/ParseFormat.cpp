...
Parser::StmtResult Parser::ParseFORMATSpec(SourceLocation Loc) {
  ...
  return Actions.ActOnFORMAT(Context, Loc, Items, UnlimitedItems, StmtLabel);
}
...
Parser::FormatItemResult Parser::ParseFORMATItems(bool IsOuter) {
  ...
  do {
    ...
    if (Item.isUsable())
      FormatList.push_back(Item.take());
  } while (ConsumeIfPresent(tok::comma));

  if (!ExpectAndConsume(tok::r_paren))
    return FormatItemResult(true);

  return Actions.ActOnFORMATFormatItemList(Context, Loc, nullptr, FormatList);
}
...
public:
  ...
  IntegerConstantExpr *ParseIntExpr(const char *DiagAfter = nullptr) {
    ...
    return IntegerConstantExpr::Create(Context,
                                       SourceRange(Loc, getCurrentLoc()), Str);
  }
...
Parser::FormatItemResult Parser::ParseFORMATItem() {
  ...
  FormatDescriptorParser FDParser(Context, Diag, TheLexer, Tok);
  ...
  IntegerConstantExpr *W = nullptr, *MD = nullptr, *E = nullptr;
  switch (Desc) {
  // data-edit-desc
  case tok::fs_I:
  case tok::fs_B:
  ...
  case tok::fs_Z:
    ...
    FDParser.MustBeDone();
    return Actions.ActOnFORMATIntegerDataEditDesc(Context, Loc, Desc, PreInt, W,
                                                  MD);
  ...
  case tok::fs_G:
    ...
    if (!MD)
      break;
    if (Desc != tok::fs_F &&
        (FDParser.LexCharIfPresent('E') || FDParser.LexCharIfPresent('e'))) {
      E = FDParser.ParseIntExpr("E");
    }
    FDParser.MustBeDone();
    return Actions.ActOnFORMATRealDataEditDesc(Context, Loc, Desc, PreInt, W,
                                               MD, E);

  case tok::fs_L:
    W = FDParser.ParseIntExpr(DescriptorStr.data());
    FDParser.MustBeDone();
    return Actions.ActOnFORMATLogicalDataEditDesc(Context, Loc, Desc, PreInt,
                                                  W);
  ...
  case tok::fs_A:
    ...
    FDParser.MustBeDone();
    return Actions.ActOnFORMATCharacterDataEditDesc(Context, Loc, Desc, PreInt,
                                                    W);
  ...
  case tok::fs_X:
    if (!PreInt) {
      Diag.Report(DescriptorStrLoc,
                  diag::err_expected_int_literal_constant_before)
          << "X";
      break;
    }
    FDParser.MustBeDone();
