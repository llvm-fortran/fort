...
#ifndef LLVM_FORT_AST_EXPRVISITOR_H
#define LLVM_FORT_AST_EXPRVISITOR_H
...
#define DISPATCH(NAME, CLASS)                                                  \
  ...
  RetTy Visit(PTR(Expr) E) {
    ...
    if (PTR(BinaryExpr) BinOp = dyn_cast<BinaryExpr>(E)) {
      ...
      case BinaryExpr::Eqv:
        DISPATCH(BinaryExprEqv, BinaryExpr);
      case BinaryExpr::Neqv:
        DISPATCH(BinaryExprNeqv, BinaryExpr);
      case BinaryExpr::Or:
        DISPATCH(BinaryExprOr, BinaryExpr);
      case BinaryExpr::And:
        DISPATCH(BinaryExprAnd, BinaryExpr);
      case BinaryExpr::Equal:
        DISPATCH(BinaryExprEQ, BinaryExpr);
      case BinaryExpr::NotEqual:
        DISPATCH(BinaryExprNE, BinaryExpr);
      case BinaryExpr::LessThan:
        DISPATCH(BinaryExprLT, BinaryExpr);
      case BinaryExpr::LessThanEqual:
        DISPATCH(BinaryExprLE, BinaryExpr);
      case BinaryExpr::GreaterThan:
        DISPATCH(BinaryExprGT, BinaryExpr);
      case BinaryExpr::GreaterThanEqual:
        DISPATCH(BinaryExprGE, BinaryExpr);
      case BinaryExpr::Concat:
        DISPATCH(BinaryExprConcat, BinaryExpr);
      case BinaryExpr::Plus:
        DISPATCH(BinaryExprAdd, BinaryExpr);
      case BinaryExpr::Minus:
        DISPATCH(BinaryExprSub, BinaryExpr);
      case BinaryExpr::Multiply:
        DISPATCH(BinaryExprMul, BinaryExpr);
      ...
      case UnaryExpr::Not:
        DISPATCH(UnaryExprNot, UnaryExpr);
      case UnaryExpr::Plus:
...
#define BINOP_FALLBACK(NAME)                                                   \
  ...
  BINOP_FALLBACK(Eqv)
  BINOP_FALLBACK(Neqv) BINOP_FALLBACK(Or) BINOP_FALLBACK(And) BINOP_FALLBACK(EQ)
      BINOP_FALLBACK(NE) BINOP_FALLBACK(LT) BINOP_FALLBACK(LE)
          BINOP_FALLBACK(GT) BINOP_FALLBACK(GE) BINOP_FALLBACK(Concat)
              BINOP_FALLBACK(Add) BINOP_FALLBACK(Sub) BINOP_FALLBACK(Mul)
                  BINOP_FALLBACK(Div) BINOP_FALLBACK(Pow)
...
#define UNARYOP_FALLBACK(NAME)                                                 \
  RetTy VisitUnaryExpr##NAME(PTR(UnaryExpr) E) {                               \
    DISPATCH(UnaryExpr, UnaryExpr);                                            \
                      ...
                      UNARYOP_FALLBACK(Not) UNARYOP_FALLBACK(Plus)
                          UNARYOP_FALLBACK(Minus)

#undef UNARYOP_FALLBACK

      // Base case, ignore it. :)
      RetTy VisitExpr(PTR(Expr) Node) {
    return RetTy();
  }
...
class ExprVisitor : public ExprVisitorBase<make_ptr, ImplClass, RetTy> {};
