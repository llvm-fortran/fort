...
template <> struct PointerLikeTypeTraits<fort::DeclContext *> {
...
protected:
  ...
  Decl(Kind DK, DeclContext *DC, SourceLocation L)
      : NextDeclInContext(0), DeclCtx(DC), Loc(L), DeclKind(DK),
        InvalidDecl(false), HasAttrs(false), Implicit(false), ImplicitType(0),
        SubDeclKind(0) {}
...
public:
  ...
  DeclContextLookupResult() : std::pair<NamedDecl **, NamedDecl **>() {}
...
public:
  ...
  Decl::Kind getDeclKind() const { return static_cast<Decl::Kind>(DeclKind); }
...
protected:
  RecordDecl(Kind DK, DeclContext *DC, SourceLocation StartLoc,
...
class MainProgramDecl : public DeclaratorDecl, public DeclContext {
  ...
  MainProgramDecl(DeclContext *DC, const DeclarationNameInfo &NameInfo)
      : DeclaratorDecl(MainProgram, DC, NameInfo.getLoc(), NameInfo.getName(),
                       QualType()),
        DeclContext(MainProgram), Body(nullptr) {}
...
public:
  ...
  enum Attributes { NoAttributes = 0, Recursive = 1 };
...
private:
  ...
  FunctionDecl(Kind DK, FunctionKind FK, DeclContext *DC,
               const DeclarationNameInfo &NameInfo, QualType T, int Attr);

public:
  static FunctionDecl *Create(ASTContext &C, FunctionKind FK, DeclContext *DC,
                              const DeclarationNameInfo &NameInfo,
                              QualType ReturnType, int Attr = NoAttributes);
  ...
  bool isExternalArgument() const { return SubDeclKind == ExternalArgument; }
  ...
  VarDecl *getResult() const { return Result; }
...
class SelfDecl : public DeclaratorDecl {
  ...
  SelfDecl(DeclContext *DC, DeclaratorDecl *D)
      : DeclaratorDecl(Self, DC, D->getLocation(), D->getIdentifier(),
                       D->getType()),
        SelfD(D) {}

public:
  static SelfDecl *Create(ASTContext &C, DeclContext *DC, DeclaratorDecl *Self);

  DeclaratorDecl *getSelf() const { return SelfD; }
...
class OutDecl : public DeclaratorDecl {
  ...
  OutDecl(DeclContext *DC, DeclaratorDecl *D)
      : DeclaratorDecl(Out, DC, D->getLocation(), D->getIdentifier(),
                       D->getType()),
        RefD(D) {}

public:
  static OutDecl *Create(ASTContext &C, DeclContext *DC, DeclaratorDecl *Self);

  DeclaratorDecl *getDecl() const { return RefD; }
...
class IntrinsicFunctionDecl : public DeclaratorDecl {
  ...
  IntrinsicFunctionDecl(Kind DK, DeclContext *DC, SourceLocation IDLoc,
                        const IdentifierInfo *ID, QualType T,
                        intrinsic::FunctionKind Func)
      : DeclaratorDecl(DK, DC, IDLoc, ID, T), Function(Func) {}
...
public:
  ...
  intrinsic::FunctionKind getFunction() const { return Function; }
...
protected:
  ModuleDecl(DeclContext *DC, const DeclarationNameInfo &NameInfo)
      : DeclaratorDecl(Module, DC, NameInfo.getLoc(), NameInfo.getName(),
                       QualType()),
        DeclContext(Module) {}
...
protected:
  FieldDecl(Kind DK, DeclContext *DC, SourceLocation IdLoc,
            const IdentifierInfo *Id, QualType T)
...
protected:
  VarDecl(Kind DK, DeclContext *DC, SourceLocation IdLoc,
          const IdentifierInfo *ID, QualType T)
      : DeclaratorDecl(DK, DC, IdLoc, ID, T), Init(nullptr), Storage(nullptr) {}

public:
  static VarDecl *Create(ASTContext &C, DeclContext *DC, SourceLocation IDLoc,
                         const IdentifierInfo *ID, QualType T);
  ...
  void Profile(llvm::FoldingSetNodeID &ID) { Profile(ID, getIdentifier()); }
  ...
  inline bool isUnusedSymbol() const { return SubDeclKind == UnusedSymbol; }
  ...
  bool hasStorageSet() const { return Storage != nullptr; }
  void setStorageSet(StorageSet *S) { Storage = S; }
...
class CommonBlockDecl : public NamedDecl {
  ...
  CommonBlockDecl(DeclContext *DC, SourceLocation IDLoc,
...
public:
  ...
  CommonBlockSet *getStorageSet() const { return Set; }
  void setStorageSet(CommonBlockSet *S) { Set = S; }
...
/// isa<T>(DeclContext*)
template <typename To> struct isa_impl<To, ::fort::DeclContext> {
...
/// cast<T>(DeclContext*)
template <class ToTy>
struct cast_convert_val<ToTy, const ::fort::DeclContext,
                        const ::fort::DeclContext> {
